...
15. **공부 도전 종료** = 회원은 참여 기회가 복구되지 않고 처음 확정한 4주 기간이 끝나기 전에는 다시 참여할 수 없음을 명시적으로 확인한 뒤 본인의 도전 종료를 요청할 수 있다. 서버 처리 시각을 기준으로 시작 전은 `CANCELLED`, 시작 후는 `WITHDRAWN`으로 구분하며 결제 환불 등 별도 사유는 lifecycle 기록을 기준으로 표시한다. 둘 다 종료 상태이며 남은 `PENDING` 주차는 진행 중이나 집계 대기가 아닌 미진행으로 표시한다. 관리자 도전 상세에는 신규 감사 기록만 참여·종료 타임라인으로 표시하고, 확인할 수 없는 과거 기록은 추정해 채우지 않는다.
