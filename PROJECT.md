...
12. **업무 시작/종료 안내** = 활성 회원의 새 앱 실행/로그인 시 서울 06:00 기준 안내일의 첫 입장 모달을 브라우저별 1회 표시한다. 공용 PC 개인정보 보호를 위해 로그아웃하면 해당 회원의 브라우저 claim을 삭제하므로, 같은 날 다시 로그인하면 안내가 다시 표시될 수 있다. 06:00~21:59는 카메라 준비 후 공부 시작, 22:00~05:59는 야간 자율공부 시작 흐름이며 자정을 지나도 안내일은 바뀌지 않는다. 09:00 `bell.announcement`는 일정 알림음과 기존 `periodStart` 동기화만 유지하고 모달을 중복 표시하지 않는다. 22:00 안내는 별도 1회 표시하며 `계속 공부하기`를 누르면 연결된 카메라를 서버에서 검증한 뒤 기록을 바로 재개한다. 팝업 클릭 자체가 아니라 LiveKit 카메라 바인딩/검증 성공 시점부터 공부시간을 기록한다.
