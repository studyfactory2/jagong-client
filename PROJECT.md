...
6. **미리보기 캠** = 가상 화면(샘플).
7. **"현재 N명"** = 가입자 수.
8. **시간표 휴식시간** = 카메라가 켜져 있으면 공부시간 기록을 계속한다. 사용자가 `휴식 시작`을 누를 때만 `/study-time/break` 처리 후 카메라 영상만 끈다.
9. **공부 계속하기** = 카메라 영상을 먼저 다시 켠 뒤, 서버가 LiveKit 카메라를 검증하는 `/study-time/resume`으로 기록을 재개한다. 실패하면 카메라와 서버 상태를 안전한 휴식 상태로 되돌리고 `/study-time/me`로 재확인하며, 안전한 복구가 불가능하면 작업실 연결을 종료한다.
10. **자동 카메라 경고 제외** = 휴식 전환에서 오디오·화면공유·MediaPipe 자동 관리자 경고는 사용하지 않는다.
