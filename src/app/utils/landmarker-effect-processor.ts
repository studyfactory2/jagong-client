...
type AdaptiveQualityLevel = "full" | "balanced" | "economy";

type AdaptiveQualityProfile = {
...
const ADAPTIVE_QUALITY: Record<AdaptiveQualityLevel, AdaptiveQualityProfile> = {
  full: {
...
class AnimalEffectTransformer implements VideoTrackTransformer<AnimalEffectOptions> {
  ...
  private qualityLevel: AdaptiveQualityLevel = getInitialQualityLevel();
  ...
  transform(
    ...
    if (!this.shouldProcessFrame(performance.now())) {
      frame.close();
      return;
    }
    ...
    try {
      ...
      frame.close();
      this.recordFrameCost(performance.now() - frameStartedAt);
    }
  }
  ...
  private ensureWorkingCanvases() {
    ...
    const profile = ADAPTIVE_QUALITY[this.qualityLevel];
    const longestEdge = Math.max(outputCanvas.width, outputCanvas.height);
    const detectionScale = Math.min(
      1,
      profile.detectionMaxEdge / Math.max(1, longestEdge),
    );
  ...
  private applyDogBeauty(tracking: TrackingResult) {
    const profile = ADAPTIVE_QUALITY[this.qualityLevel];
    if (this.beautyUnavailable || !profile.beautyEnabled) return;
    const { outputCanvas, outputContext } = this.requireOutput();
  ...
  private recordFrameCost(frameCostMs: number) {
    ...
    if (this.qualityLevel === "full") {
      if (
        averageCost >= 20 ||
        slowFrames >= 8 ||
        slowCadenceFrames >= 18
      ) {
        this.setQualityLevel("balanced", now);
      }
      return;
    }

    if (this.qualityLevel === "balanced") {
      if (
        averageCost >= 28 ||
        slowFrames >= 12 ||
        slowCadenceFrames >= 24
      ) {
        this.setQualityLevel("economy", now);
        return;
      }
  ...
  private setQualityLevel(level: AdaptiveQualityLevel, changedAt: number) {
    ...
    if (!ADAPTIVE_QUALITY[level].beautyEnabled) {
      this.beautyRenderer?.destroy();
      this.beautyRenderer = null;
    }
  ...
  private shouldProcessFrame(now: number) {
    const targetFps = ADAPTIVE_QUALITY[this.qualityLevel].targetFps;
